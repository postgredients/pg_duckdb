...
-- duckdb.allowed_endpoint_suffixes must match on a dot boundary, so an allowed
-- suffix like 'yandex.net' does not also accept an unrelated host such as
-- 'evilyandex.net' (SSRF guard).
SET duckdb.allowed_endpoint_suffixes = 'yandex.net';
-- Exact host and real subdomains are allowed.
CREATE SERVER endpoint_exact_ok TYPE 's3' FOREIGN DATA WRAPPER duckdb OPTIONS (endpoint 'yandex.net');
CREATE SERVER endpoint_dot_ok TYPE 's3' FOREIGN DATA WRAPPER duckdb OPTIONS (endpoint 's3.yandex.net');
-- No dot boundary before the suffix: must be rejected.
CREATE SERVER endpoint_bypass TYPE 's3' FOREIGN DATA WRAPPER duckdb OPTIONS (endpoint 'evilyandex.net');
-- Suffix as a left-hand label of an attacker domain: must be rejected.
CREATE SERVER endpoint_appended TYPE 's3' FOREIGN DATA WRAPPER duckdb OPTIONS (endpoint 'yandex.net.attacker.example');
set client_min_messages=WARNING; -- suppress NOTICE that include username
DROP SERVER endpoint_exact_ok, endpoint_dot_ok CASCADE;
reset client_min_messages;
RESET duckdb.allowed_endpoint_suffixes;
